...
def sort_key(filename):
    ...
    segment_number = int(parts[3].split(".")[0])  # Czwórka po split na "_" i ".csv", np. "1" z "segmen1.csv"
...
class Dataset:
    def __init__(self, dirpath: str, condition_dir_name: str = "condition"):
        ...
        self.condition: List[pd.DataFrame] = [
            ...
            for file in sorted(os.listdir(condition_dirpath), key=sort_key)  # Sortowanie plików
        ]

        self.control: List[pd.DataFrame] = [
            pd.read_csv(os.path.join(control_dirpath, file))  # Wczytanie pliku
            for file in sorted(os.listdir(control_dirpath), key=sort_key)  # Sortowanie plików
        ]
